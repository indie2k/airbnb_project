...
기능적 요구사항
1. 호스트(방주인)가 임대할 방을 등록한다.
2. 호스트(방주인)이 등록한 방의 내용을 수정/삭제 할 수 있다
3. 고객이 방을 예약한다.
4. 예약과 동시에 결재가 이루어 진다.
5. 고객 결재 완료 시, 예약이 확정된다.
6. 확정 시 고객에게 알림 메시지가 전송된다.
7. 고객이 예약을 취소할 수 잇다.
8. 고객이 후기를 등록/수정/삭제 할 수 있다.
9. 등록된 모든 방의 상태, 예약가능 등 확인할 수 있는 페이지가 제공된다 (CQRS)

비기능적 요구사항
1. 트랜잭션
    1. 미 결재 방 예약은 거래가 성립되지 않아야 한다.(Sync 호출) 
1. 장애격리
    1. 호스트 방 등록/수정/삭제 기능 수행여부와 관계없이, 고객 방 예약/결재/취소 기능은 항상 처리 가능하다.    
    1. 결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다.
1. 성능
    1. 고객은 방 Status 확인을 한 페이지에서 종합 확인 가능하다(설명/리뷰건수/상태 등, CQRS)    
    1. 방 예약 상태가 변경될 때마다 메시지로 알려준다
...
![image](https://user-images.githubusercontent.com/15603058/119300858-6c21b300-bc9c-11eb-9b3f-c85aff51658f.png)
    ...
    - Core Domain : Room, Reservation, Payment
    - Supporting Domain : Message (Reservation Event 에 따른 대고객 알림 서비스)
    - General Domain : Payment 독립된 결재 서비스로 외부 연계 가능, Reservation confirm/cancel 트리거 역할
